module.exports = client => {
  ...
  if (debug === true) {
    ...
    console.log(chalk.magenta('[Micium Debug] ') + chalk.green('Debug Mode: ') + chalk.cyan('true'))
    console.log(chalk.magenta('[Micium Debug] ') + chalk.green('Debug Axios Mode: ') + chalk.cyan(debugerror))
    console.log(chalk.magenta('[Micium Debug] ') + chalk.green('Resource: ') + chalk.cyan(resource))
    console.log(chalk.magenta('[Micium Debug] ') + chalk.green('Custom Status: ') + chalk.cyan(enablecs))
    console.log(chalk.magenta('[Micium Debug] ') + chalk.green('Enable Timestamp: ') + chalk.cyan(enablets))
    console.log(chalk.magenta('[Micium Debug] ') + chalk.green('Enable Description: ') + chalk.cyan(enabledesc))
    console.log(chalk.magenta('[Micium Debug] ') + chalk.green('Enable Footer: ') + chalk.cyan(enablef))
  }
  ...
  if (enablecs === true) {
    ...
    client.user.setActivity('Micium', { type: 'WATCHING' })
  }
  ...
  setInterval(() => {
    if (isNaN(time)) return console.log(chalk.cyan('[Micium Wings] ') + chalk.red(time + ' is not a number!'))
    if (!hosturl.includes('.')) return console.log(chalk.cyan('[Micium Wings] ') + chalk.red(hosturl + ' is invalid url!'))
    if (adminapikey.length < 48) return console.log(chalk.cyan('[Micium Wings] ') + chalk.red('Invalid Admin Apikey!!'))
    ...
    axios(api + '/application/nodes/', {
        ...
        axios(api + '/application/nodes/' + id + '?include=servers,location,allocations', {
          ...
          axios(api + '/application/nodes/' + id + '/configuration', {
            ...
            axios(node.data.attributes.scheme + '://' + node.data.attributes.fqdn + ':' + node.data.attributes.daemon_listen + '/api/servers', {
              ...
              console.log(chalk.cyan('[Micium Wings] ') + chalk.red(node.data.attributes.name + ' is down!'))
              if (debugerror === true) console.log(chalk.magenta('[Micium Debug] ') + chalk.red(err) + chalk.cyan(' Need Support? https://discord.gg/zv6maQRah3'))
            ...
            console.log(chalk.magenta('[Micium Debug] ') + chalk.red(err) + chalk.cyan(' Need Support? https://discord.gg/zv6maQRah3'))
          })
        }).catch((err) => {
          console.log(chalk.magenta('[Micium Debug] ') + chalk.red(err) + chalk.cyan(' Need Support? https://discord.gg/zv6maQRah3'))
        })
      ...
      axios(api + '/application/servers', {
        ...
        console.log(chalk.cyan('[Micium Wings] ') + chalk.red('Panel is down'))
        if (debugerror === true) console.log(chalk.magenta('[Micium Debug] ') + err)
      })
      ...
      if (nodes === undefined) {
        ...
        console.log(chalk.cyan(['[Micium Wings] ']) + chalk.yellow(checking + ' Please wait ' + time + ' seconds'))
      }
      ...
      console.log(chalk.cyan('[Micium Wings] ') + chalk.green('Posted Stats'))
      if (panel !== null) console.log(chalk.cyan('[Micium Wings] ') + chalk.green('Stats Updated'))
      console.log(chalk.cyan('[Micium Wings] ') + chalk.green('Updating Stats in ' + time + ' Seconds'))
